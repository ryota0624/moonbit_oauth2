...
**mock-oauth2-server を使った高速テスト：**

```bash
# MoonBit 統合テスト CLI を実行（自動的に mock-oauth2 を起動）
...
|------|-------------------|----------|
| **起動時間** | 即座（< 5秒） | 約30秒 |
| **設定の複雑さ** | 不要（即座に利用可能） | 自動化済み（setup_keycloak.sh） |
| **OAuth2 対応範囲** | 基本フロー | 全フローサポート + PKCE |
...
| `run_integration_test_cli.sh` | 開発用統合テスト（mock-oauth2） | 約10秒 | ⭐ |
| `run_integration_tests.sh` | CI 用テスト（moon test + mock-oauth2） | 約15秒 | ⭐ |
