...
### 2. [統合テストガイド](./integration_test_guide.md)

**概要**: Docker を使用した自動統合テストの実行方法

**内容**:
- mock-oauth2-server を使用したクイックテスト
- 統合テスト CLI の使用方法
- CI/CD パイプラインへの組み込み方法

**対象者**:
- 自動テストを実装する開発者
- CI/CD を設定する DevOps エンジニア
...
# 3. 手順書に従って設定
# docs/testing/keycloak_verification_guide.md を参照
```

### Option 3: 簡易テスト（mock-oauth2-server）

開発初期段階や簡単な動作確認の場合：

```bash
# 1. mock-oauth2-server を起動
...
| 項目 | mock-oauth2-server | Keycloak |
|------|-------------------|----------|
| **起動時間** | 即座（< 5秒） | 約30秒 |
| **設定の複雑さ** | 不要（即座に利用可能） | 必要（レルム、クライアント等） |
| **OAuth2 対応範囲** | 基本フロー | 全フローサポート |
| **ユーザー管理** | 固定（事前設定） | 完全な管理機能 |
| **トークン形式** | 簡易的な JWT | 本番相当の JWT |
| **適用場面** | 開発・単体テスト | 統合テスト・本番前検証 |
| **リソース使用量** | 軽量 | 中程度（PostgreSQL 含む） |

**推奨**:
- 🟢 **開発中**: mock-oauth2-server（高速・簡単）
- 🟡 **統合テスト**: Keycloak（本番相当）
- 🔴 **本番前検証**: Keycloak（完全な機能テスト）
...
| `test_keycloak_flows.sh` | OAuth2 フローの自動テスト | 約30秒 |
| `run_integration_test_cli.sh` | 統合テスト CLI の実行 | 約10秒 |
